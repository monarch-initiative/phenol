...
public class PairwiseLinSimilarityTest extends VegetableOntologyTestBase {

  PairwiseLinSimilarity<VegetableTerm, VegetableTermRelation> similarity;
  ...
  public void setUp() {
    ...
    similarity =
        new PairwiseLinSimilarity<>(new PairwiseResnikSimilarity<>(ontology, informationContent));
  }

  @Test
  public void testComputeSimilarities() {
    assertEquals(0.0, similarity.computeScore(idBeet, idCarrot), 0.01);
    assertEquals(0.539, similarity.computeScore(idBlueCarrot, idCarrot), 0.01);
