...
public class PairwiseJiangSimilarityTest extends VegetableOntologyTestBase {

  PairwiseJiangSimilarity<VegetableTerm, VegetableTermRelation> similarity;
  ...
  public void setUp() {
    ...
    similarity =
        new PairwiseJiangSimilarity<>(new PairwiseResnikSimilarity<>(ontology, informationContent));
  }

  @Test
  public void testComputeSimilarities() {
    assertEquals(0.5522, similarity.computeScore(idBeet, idCarrot), 0.01);
    assertEquals(0.5906, similarity.computeScore(idBlueCarrot, idCarrot), 0.01);
    assertEquals(0.5522, similarity.computeScore(idPumpkin, idCarrot), 0.01);
    assertEquals(0.5522, similarity.computeScore(idLeafVegetable, idCarrot), 0.01);
  }
